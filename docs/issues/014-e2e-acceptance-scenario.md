...
项目定位为求职/作品集叙事（见 ADR 0008 背景），本 Issue 是核心 demo：一个**可重复执行**的端到端验收场景，证明 Harness 的完整闭环——act → L2 结构化验证 → Vision 视觉验证 → checkpoint 保存 → 轨迹记录。"要不要做下去"只能对着具体的高强度场景回答，本场景就是那个标尺。
