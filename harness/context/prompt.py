...
class SystemContextProvider(ContextProvider):
    ...
    AGENT_IDENTITY = (
        ...
        "\n"
        "  氛围优先 → activate_skill(\"match-atmosphere\") + activate_skill(\"color-diagnostics\")\n"
        "    参考图驱动的整体场景氛围调整：光照、天空、雾、云、后处理\n"
        "    ⚠ 氛围匹配必须同时激活两个 Skill：\n"
        "       match-atmosphere → 提供 5 步调整工作流\n"
        "       color-diagnostics → 提供颜色诊断决策树（症状→根因→修复优先级）\n"
        "    只激活 match-atmosphere 不够——没有诊断框架你会陷入试错调整。\n"
        "\n"
